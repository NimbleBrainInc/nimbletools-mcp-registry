...
class ServerValidator:
    ...
    def validate_server(self, server_path: Path) -> bool:
        ...
        self._validate_image(server_data)
    ...
    def _validate_image(self, server_data: Dict[str, Any]) -> None:
        """Validate Docker image reference"""
        deployment = server_data.get('deployment', {})
        image = deployment.get('image', '')
        
        if not image:
            self.errors.append("Docker image is required")
            return
        
        # Basic image format validation
        if not re.match(r'^[a-z0-9-_./]+:[a-zA-Z0-9-_.]+$', image):
            self.errors.append("Invalid Docker image format")
        
        # Check if it's a NimbleBrain image (recommendation)
        if not image.startswith('nimblebrain/'):
            self.warnings.append("Consider using 'nimblebrain/' prefix for official registry images")
